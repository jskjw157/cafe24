...
- Use `/logout` to clear your credentials, then `/quit` (or `/exit`) to leave the CLI once you finish.

## Create your own slash commands with custom prompts

Custom prompts turn Markdown files into reusable slash commands that you trigger with `/prompts:<name>`. Custom prompts require explicit invocation. You can't share them in repositories. If you want to share a prompt or want Codex to implicitly invoke it, [check out skills](/codex/skills).
...
- Extend Codex capabilities with shareable [agent skills](/codex/skills).
