        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    aHtml.push('<div class="view"><p>パスワード <input type="password" id="secure_password" name="secure_password" onkeydown="if (event.keyCode == 13) '+data.action_pass_submit+'"> <input type="button" value="確認" onclick="'+data.action_pass_submit+'"></p></div>');
                    aHtml.push('</form>');
                } else {
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="#none" onclick="EC_PRODUCT_FRONT_BOARD_QNA.modify(' + data.no + ', ' + iProductNo + ');" class="btnNormal">修正</a>');
                    }
                    aHtml.push('</span></p>');
                    aHtml.push('<p class="ec-base-button"><span class="gRight">');
                    aHtml.push('<a href="#none" class="btnNormalFix sizeS ' + data.read['report_open_btn'] +'" onclick="'+ data.read['report_open_layer_action'] +'">報告</a> ');
                    aHtml.push('<a href="#none" class="btnNormalFix sizeS ' + data.read['block_request_btn'] +'" onclick="'+ data.read['block_action'] +'">ブロック</a>');
                    aHtml.push('<a href="#none" class="btnNormalFix sizeS ' + data.read['unblock_request_btn'] +'" onclick="'+ data.read['unblock_action'] +'">ブロック 解除</a>');
                    aHtml.push('</span</p>');
                    aHtml.push('</div>');
                    ...
                    if (data.comment != undefined && data.comment.length != undefined) {
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                aHtml.push('<span class="grade '+data.use_point+'"><img src="//img.echosting.cafe24.com/skin/base_ja_JP/board/ico_point'+data.comment[i]['comment_point_count']+'.gif" alt="'+data.comment[i]['comment_point_count']+'点" /></span>');
                                if (data.comment[i]['comment_reply_display'] == true) {
                                    aHtml.push('<span class="button">'+'<a href="#none" onclick="'+data.comment[i]['action_comment_reply']+'" class="btnNormal">コメント <img src="//img.echosting.cafe24.com/skin/base/common/btn_icon_reply.gif" alt="" /></a>'+'</span>');
                                }
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<div class="user"><div class="nameArea">名前 '+data.comment_write['comment_name']+' パスワード '+data.comment_write['comment_password']);
                        if (data.comment_write['comment_secret_display'] == true) {
                            aHtml.push('<label class="secret">'+data.comment_write['secure']+' シークレットコメント</label>');
                        }
                        ...
                        aHtml.push('</div>');
                        aHtml.push(''+data.comment_write['comment']+'<a href="#none" onclick="'+data.comment_write['action_comment_insert']+'" class="btnEm sizeL">確認</a></div>');
                        ...
                        aHtml.push('<p class="captcha '+data.comment_write['use_captcha']+'">'+data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+data.comment_write['captcha']+'<img src="//img.echosting.cafe24.com/skin/base/common/ico_info.gif" alt="" /> *文字の前後に余分なスペースを空けないようにして入力してください。(大小文字を区別)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<div class="writer">');
                        aHtml.push('<div class="user"><div class="nameArea">名前 '+data.comment_reply['comment_name']+' パスワード '+data.comment_reply['comment_password']);
                        if (data.comment_reply['comment_secret_display'] == true) {
                            aHtml.push('<label class="secret">'+data.comment_reply['secure']+' シークレットコメント</label>');
                        }
                        aHtml.push('<p class="ec-base-help '+data.comment_write['password_rule_help_display_class']+'">半角英字(大文字・小文字)/数字/特殊文字のうち、２種類以上の組み合わせで10～16文字</p>');
                        aHtml.push('</div>');
                        aHtml.push(''+data.comment_reply['comment']+'<a href="#none" onclick="'+data.comment_reply['action_comment_insert']+'" class="btnEm sizeL">確認</a></div>');
                        ...
                        aHtml.push('<p class="captcha '+data.comment_reply['use_captcha']+'">'+data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+data.comment_reply['captcha']+'<img src="//img.echosting.cafe24.com/skin/base/common/ico_info.gif" alt="" /> *文字の前後に余分なスペースを空けないようにして入力してください。(大小文字を区別)');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_secret != undefined) {
                        ...
                        aHtml.push('<p>パスワード : '+data.comment_secret['secure_password']);
                        aHtml.push(' <a href="#none" onclick="'+data.comment_secret['action_secret_submit']+'" class="btnNormal">確認</a>');
                        aHtml.push(' <a href="#none" onclick="'+data.comment_secret['action_secret_cancel']+'" class="btnNormal">キャンセル</a></p>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                }
