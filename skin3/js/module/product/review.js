            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="#none" onclick="EC_PRODUCT_FRONT_BOARD_REVIEW.modify(' + data.no + ', ' + iProductNo + ');" class="btnNormal">修正</a>');
                    }
                    aHtml.push('</span></p>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<p class="captcha '+data.comment_write['use_captcha']+'">'+data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+data.comment_write['captcha']+'<img src="//img.echosting.cafe24.com/skin/base/common/ico_info.gif" alt="" /> スペースを空けずに入力してください。(大小文字を区別)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<p class="captcha '+data.comment_reply['use_captcha']+'">'+data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+data.comment_reply['captcha']+'<img src="//img.echosting.cafe24.com/skin/base/common/ico_info.gif" alt="" /> スペースを空けずに入力してください。(大小文字を区別)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
