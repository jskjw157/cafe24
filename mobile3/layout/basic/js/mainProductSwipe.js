...
var $S = {
    ...
    makePagenate : function() {
        ...
        aBtn.push( '<button type="button" class="prev" onclick="$' + this.sModuleSwipe + '_slider_' + this.iActive + '.prev();return false;"><span>前へ</span></button>' );
        aBtn.push( '<span id="' + this.sModule + '-swipe-button-' + this.iActive + '">' +  this.aCircle.join('') +'</span>' );
        aBtn.push( '<button type="button" class="next" onclick="$' + this.sModuleSwipe + '_slider_' + this.iActive + '.next();return false;"><span>次へ</span></button>' );
        aBtn.push( '</div>' );
