            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="/board/product/modify.html' + data.read['param_modify'] + '&link_product_no=' + iProductNo + '" class="btnNormal mini"><img src="//img.echosting.cafe24.com/skin/mobile_ja_JP/board/btn_comment_modify.png" width="13" height="13" alt="修正"></a>');
                    }
                    aHtml.push('</div>');
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                ...
                                if (data.comment[i]['comment_reply_display'] == true) {
                                    ...
                                    aHtml.push('<a href="#none" class="btnNormal mini" onclick="QNA.comment_reply_view('+data.comment[i]['comment_no']+')"><img src="//img.echosting.cafe24.com/skin/mobile_ja_JP/board/btn_comment_view.png" width="13" height="13" alt="コメントに対するコメント"> <em>('+data.comment[i]['comment_reply_count']+')</em></a>');
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('<div class="captcha ' +data.comment_write['use_captcha']+ '"><span class="img"></span><div class="form">' +data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+data.comment_write['captcha']+ '<p>左側の文字をスペースを空けずに入力してください。<br>(大・小文字を区別)</p></div></div>');
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('<div class="captcha ' +data.comment_reply['use_captcha']+ '"><span class="img"></span><div class="form">' +data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+data.comment_reply['captcha']+ '<p>左側の文字をスペースを空けずに入力してください。<br>(大・小文字を区別)</p></div></div>');
